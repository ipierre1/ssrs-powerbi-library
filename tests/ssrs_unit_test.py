...
class TestSSRSDataSourceManager(unittest.TestCase):
    ...
    def test_test_item_data_source_connection(self):
        ...
        with patch.object(self.ds_manager, 'get_item_data_sources', return_value=[mock_ds]):
            ...
            self.assertIn('TestDS', results)
            self.assertTrue(results['TestDS'])
    ...
    def test_test_item_data_source_connection_specific_ds(self):
        ...
        mock_ds1 = RsDataSource(name='DS1', data_source_type='SQL', connection_string='Server=test1;')
        mock_ds2 = RsDataSource(name='DS2', data_source_type='SQL', connection_string='Server=test2;')
        
        with patch.object(self.ds_manager, 'get_item_data_sources', return_value=[mock_ds1, mock_ds2]):
            mock_response = Mock()
            mock_response.status_code = 200
            self.mock_client._make_request.return_value = mock_response
            self.mock_client._get_catalog_item_path.return_value = '/TestReport'
            ...
            results = self.ds_manager.test_item_data_source_connection('/TestReport', 'DS2')
            
            self.assertEqual(len(results), 1)
            self.assertIn('DS2', results)
            self.assertNotIn('DS1', results)
