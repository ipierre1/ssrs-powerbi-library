...
class TestSSRSSecurityManager(unittest.TestCase):
    ...
    def test_get_data_model_roles_success(self):
        ...
        mock_response.json.return_value = {
            'value': [
                {
                    'ModelRoleName': 'Sales Manager',
                    'ModelRoleId': 'role-id-1'
                },
                {
                    'ModelRoleName': 'Sales Rep',
                    'ModelRoleId': 'role-id-2'
                }
            ]
        }
        self.mock_client._make_request.return_value = mock_response
    ...
    def test_get_data_model_role_assignments_success(self):
        ...
        mock_response.json.return_value = {
            'value': [
                {
                    'GroupUserName': 'DOMAIN\\user1',
                    'DataModelRoles': ['role-id-1']
                },
                {
    ...
    def test_set_data_model_role_assignments_success(self):
        ...
        result = self.security_manager.set_data_model_role_assignments('report-id', assignments)
    ...
    def test_create_role_mapping(self):
        ...
        expected = {
            'source-role-1': 'target-role-1',
            'source-role-2': 'target-role-2'
        }
        self.assertEqual(result, expected)
    ...
    def test_migrate_data_model_role_assignments_success(self, mock_set, mock_get_assignments, 
                                                       mock_get_roles, mock_get_id):
    ...
    @patch.object(SSRSSecurityManager, 'get_data_model_role_assignments')
    @patch.object(SSRSSecurityManager, 'set_data_model_role_assignments')
    def test_add_user_to_role_new_user(self, mock_set, mock_get_assignments, 
                                     mock_get_roles, mock_get_id):
    ...
    @patch.object(SSRSSecurityManager, 'get_data_model_role_assignments')
    @patch.object(SSRSSecurityManager, 'set_data_model_role_assignments')
    def test_remove_user_from_role_all_roles(self, mock_set, mock_get_assignments, mock_get_id):
        ...
        result = self.security_manager.remove_user_from_role(
            '/test/report',
            'DOMAIN\\user1',
            None  # Retirer de tous les rôles
        )
    ...
    @patch.object(SSRSSecurityManager, 'get_data_model_roles')
    @patch.object(SSRSSecurityManager, 'get_data_model_role_assignments')
    def test_list_role_assignments(self, mock_get_assignments, mock_get_roles, mock_get_id):
...
class TestSecurityTypes(unittest.TestCase):
    ...
    def test_security_role_type_enum(self):
        ...
        self.assertEqual(SecurityRoleType.SYSTEM_ADMINISTRATOR.value, "System Administrator")
