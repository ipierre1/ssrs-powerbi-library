...
def example_rls_management():
    ...
    if success:
        ...
        print(f"❌ Échec de la migration RLS")
    ...
    for report_path in reports:
        ...
        try:
            ...
            if user_to_add in existing_assignments:
                print(f"    ℹ️  Utilisateur déjà assigné avec rôles: {existing_assignments[user_to_add]}")
            ...
            if success:
                print(f"    ✅ Utilisateur ajouté avec succès")
                success_count += 1
            else:
                print(f"    ❌ Échec de l'ajout de l'utilisateur")
...
def example_advanced_security_operations():
    ...
    for source, target in source_reports.items():
        ...
        try:
            # Migrer RLS
            rls_success = security_manager.migrate_data_model_role_assignments(source, target)
            
            # Migrer politiques
            policy_success = security_manager.migrate_catalog_item_policies(source, target)
            
            if rls_success and policy_success:
                print(f"  ✅ Synchronisation complète réussie")
            elif rls_success:
                print(f"  ⚠️  RLS synchronisé, échec des politiques")
            elif policy_success:
                print(f"  ⚠️  Politiques synchronisées, échec RLS")
            else:
                print(f"  ❌ Échec complet de la synchronisation")
        ...
        for group_name, config in security_template.items():
            ...
            for user in config["users"]:
                try:
                    ...
                    if success:
                        print(f"    ✅ {user} ajouté avec rôles: {', '.join(config['roles'])}")
                    else:
