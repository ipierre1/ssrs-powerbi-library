...
class SSRSDataSourceManager:
    ...
    def get_item_data_sources(self, item_path: str) -> List[RsDataSource]:
        ...
        endpoint = f"CatalogItems(Path='{formatted_path}')?&$expand=DataSources,Properties"
        ...
        try:
            ...
            for ds_data in data.get('DataSources', []):
                data_sources.append(RsDataSource(
                    id=ds_data.get('Id'),
    ...
    def test_item_data_source_connection(self, item_path: str, data_source_name: str = None) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            if data_source_name:
                data_sources = [ds for ds in data_sources if ds.name == data_source_name]
                if not data_sources:
                    logger.warning(f"Data source '{data_source_name}' not found in item: {item_path}")
                    return {}
            ...
            for ds in data_sources:
                try:
                    ...
                    results[ds.id] = response.status_code == 200 and data.get('IsSuccessful', False)
                    
                    logger.info(f"Data source '{ds.connection_string}' ({ds.id}) test result: {results[ds.id]}")
    ...
    def test_data_source_connection(self, item_path: str, data_source: RsDataSource = None) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                # This is a basic connectivity test
                result = {
                    'status': response.status_code == 200 and data.get('IsSuccessful', False),
                    'error': data.get('ErrorMessage', None)
                }
                
                logger.info(f"Data source '{data_source.connection_string}' ({data_source.id}) test result: {result['status']}")
            ...
            logger.error(f"Failed to test data source '{data_source.id}' for {item_path}: {str(e)}")
            raise
    
    def set_item_data_source(self, 
                           item_path: str, 
                           data_sources: List[RsDataSource]) -> bool:
        """
        ...
        try:
                ...
                if ds.credentials:
                    if isinstance(ds.credentials, CredentialsByUser):
                        ...
                        if ds.credentials.domain:
                            ds_dict['UserName'] = f"{ds.credentials.domain}\\{ds.credentials.username}"
                        ...
                        ds_dict['WindowsCredentials'] = ds.credentials.windows_credentials
                        if ds.credentials.domain:
                            ds_dict['UserName'] = f"{ds.credentials.domain}\\{ds.credentials.username}"
            ...
            # Update data sources
            response = self.client._make_request('PUT', endpoint, json={'value': ds_payload})
            
            success = response.status_code in [200, 204]
            logger.info(f"Set data sources for {item_path}: {'Success' if success else 'Failed'}")
